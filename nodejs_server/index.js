...
const MONGO_URL = process.env.MONGODB_URL || "mongodb://localhost:27017/pvi_messages";
...
const LARAVEL_API_VERIFY_TOKEN_URL = "http://127.0.0.1:8000/api/chat/verify-token";
...
io.on("connection", async socket => {
    ...
    if (!token) {
        console.log("Connection attempt without token. Disconnecting.");
    ...
    try {
        const response = await axios.post(LARAVEL_API_VERIFY_TOKEN_URL, {
            api_token: token
        });
        ...
        if (response.data && response.data.success) {
            ...
            socket.mysql_user_id = userData.mysql_user_id;
            ...
            try {
                const userRooms = await ChatRoom
                    .find({ participant_mysql_user_ids: socket.mysql_user_id })
                    .sort({ last_message_timestamp: -1 });
                socket.emit("userRoomsList", userRooms.map(r => r.toObject()));
            } catch (err) {
            ...
            socket.on("createRoom", async data => {
                ...
                if (!data.participantIds.includes(socket.mysql_user_id))
                    data.participantIds.push(socket.mysql_user_id);
                ...
                try {
                    ...
                    console.log(`User ${socket.mysql_user_id} created and joined room: ${savedRoom.name}`
                        + `(ID: ${savedRoom._id})`);
                        ...
                        if (participantId !== socket.mysql_user_id) {
                            ...
                            if (targetSocketId) {
                                ...
                                if (targetSocket) {
                                    ...
                                    console.log(`User ${participantId} (Socket ${targetSocketId}) auto-joined new room ${roomObject._id}`);
                                }
                            }
                        }
                    }
            ...
            socket.on("joinRoom", async data => {
                ...
                try {
                    ...
                    if (room && room.participant_mysql_user_ids.includes(socket.mysql_user_id)) {
                        ...
                        const messages = await Message.find({ room_id: data.roomId })
                            .sort({ timestamp: 1 })
                            .limit(50);
            ...
            socket.on("sendMessage", async data => {
                ...
                if (!socket.rooms.has(roomId)) {
                    console.warn(`User ${socket.mysql_user_id} (socket ${socket.id}) tried to send to room `
                ...
                try {
                    ...
                    console.log(`Message from ${socket.mysql_user_id} to room ${roomId} broadcasted `
                    ...
                    for (const participantId of room.participant_mysql_user_ids) {
                        ...
                        if (targetSocketId) {
                            ...
                            if (targetSocket && !targetSocket.rooms.has(roomId)) {
                                ...
                                console.log(`Sent new notification to user ${participantId} (socket ${targetSocketId}) for room ${roomId}`);
                            }
                        }
                    }
                } catch (err) {
            ...
            socket.on("addMembersToRoom", async data => {
                try {
                    ...
                    if (!roomId || !newUserIdsToAdd || !Array.isArray(newUserIdsToAdd) || newUserIdsToAdd.length === 0)
                        return socket.emit('roomError', { message: 'Invalid data for adding members (roomId and newUserIdsToAdd are required).' });
                    ...
                    if (uniqueNewUserIds.length === 0)
                        return socket.emit('roomError', { message: 'No new valid members to add, or selected users are already in the room.' });
                    ...
                    const existingMembersToNotify = roomObject.participant_mysql_user_ids
                        .filter(id => !uniqueNewUserIds.includes(id));
                    ...
                    uniqueNewUserIds.forEach(newMemberId => {
                        ...
                        if (targetSocketId) {
                            ...
                            if (targetSocketInstance) {
                                ...
                                console.log(`User ${newMemberId} (Socket ${targetSocketId}) was added to room ${roomObject._id} and joined its Socket.IO channel.`);
                            }
                        }
                    });
                    ...
                    console.log(`Users [${uniqueNewUserIds.join(', ')}] added to room ${roomId} by user ${socket.mysql_user_id}`);
                } catch (error) {
            ...
            console.log("Token verification by Laravel failed. Disconnecting.");
            socket.disconnect(true);

            return;
        }
    } catch (error) {
        console.error('Error verifying token with Laravel:', error.response ? error.response.data : error.message);
